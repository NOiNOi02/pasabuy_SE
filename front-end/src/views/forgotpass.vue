<template class="bg-gray-bgcolor font-nunito">
  <div class="absolute vs:left-5 left-12 top-3">
    ...
    <label
      for=""
      class="absolute text-xl font-black tracking-widest top-4 left-16 font-raleway text-red text-red-buttons"
    >
      pasaBUY
    </label>
  </div>
  ...
  <div
    id="forgotPassword"
    class="flex items-center justify-center pt-20 pb-32 bg-gray-100"
  >
    <div
      class="w-4/12 my-12 overflow-hidden text-center bg-white shadow-md vs:w-10/12 flex-grow-1 md:w-4/12 rounded-xl"
    >
      <div class="px-10 py-16">
        <h1 class="mb-5 space-x-1 space-y-1 text-2xl font-bold">
          Forgot Password
        </h1>
          ...
          <div class="mb-2 -mx-1">
            <div>
              <input
                ...
                type="email"
                required
                class="relative block w-full h-12 px-3 py-2 mb-6 text-gray-900 placeholder-gray-500 bg-gray-200 border rounded-lg appearance-none focus:outline-none focus:shadow-outline-blue focus:border-blue-300 focus:z-10 sm:text-sm"
                placeholder="Email"
  ...
  methods: {
    sendEmail() {
      api.get("sanctum/csrf-cookie").then(() => {
        api
          .post("/api/password/email", this.forgot)
          .then((res) => {
